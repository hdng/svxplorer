...
~hdang/apps/bedtools2/bin/pairToBed -type neither -a output/all.bedpe -b annot/hg38/longranger-blacklist.bed > output/all.black-list-filtered.bedpe
...
~hdang/apps/bedtools2/bin/pairToPair -type notboth -is -a output/all.black-list-filtered.bedpe -b annot/hg38/longranger-segdups.bedpe > output/all.bedpe; rm -f output/all.black-list-filtered.bedpe
...
Rscript /gscmnt/gc5105/research/hdang/SU2C/svxplorer/genome-to-sliding-window.r annot/hg38/chromsize.tsv 100000 1000 output/genome.win-100kb.step-1kb.bed
# 2b. for visualization, larger step and window, less data points
Rscript /gscmnt/gc5105/research/hdang/SU2C/svxplorer/genome-to-sliding-window.r annot/hg38/chromsize.tsv 3000000 1000000 output/genome.win-3mb.step-1mb.bed

### 3. Overlap break points with sliding windows
~hdang/apps/bedtools2/bin/intersectBed -wao -a output/genome.win-100kb.step-1kb.bed -b output/bp.bed > output/genome.win-100kb.step-1kb.bed.overlap.tsv
...
Rscript3.1.2 /gscmnt/gc5105/research/hdang/SU2C/svxplorer/summarize-sample-count.r output/genome.win-100kb.step-1kb.bed.overlap.tsv output T

### 5. Find peaks
Rscript3.1.2 /gscmnt/gc5105/research/hdang/SU2C/svxplorer/find-peaks.r output/count.rds ALL ALL 100 5 output/peaks
...
~hdang/apps/bedtools2/bin/intersectBed -wo -a output/peaks/peaks.bed -b output/genes.flank-2k.bed > output/peaks-vs-genes.overlap.tsv
...
for k in 1000 2000 5000 10000 50000 100000; do ~hdang/apps/bedtools2/bin/intersectBed -wo -a output/genes.overlap-peaks.$k.bed -b output/bp.bed | cut -f4,7-10 | sed 's/\//\t/' > output/genes.overlap-peaks.$k.overlap-bp.tsv; done
...
( echo -e "TTC28\nTRNAA-AGC"; cat annot/blacklisted_genes.txt; ~hdang/apps/bedtools2/bin/intersectBed -wo -a output/genes.overlap-peaks.50000.bed -b annot/hg38/longranger-blacklist.bed | cut -f4 | sort | uniq | grep -Pv "^PTEN$" | grep -Pv "^ATM$" ) > output/genes.blacklist.txt
...
for k in 1000 2000 5000 10000 50000 100000; do cat output/genes.blacklist.txt | ~hdang/scripts/select-rows-v2.pl 0 output/genes.overlap-peaks.$k.overlap-bp.num-samples.tsv 0 1 > output/genes.overlap-peaks.$k.overlap-bp.num-samples.filtered.tsv; cat output/genes.overlap-peaks.$k.overlap-bp.num-samples.filtered.tsv | ~hdang/scripts/select-rows-v2.pl 0 output/genes.overlap-peaks.$k.overlap-bp.tsv 0 > output/genes.overlap-peaks.$k.overlap-bp.filtered.tsv; done
