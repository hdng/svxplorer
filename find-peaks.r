# Run peakPick on sample count/pct generated by sliding window
...
in.count.file = args[1]
svtype = args[2]
chrs = unlist(strsplit(args[3], ','))
peakPick.win = as.numeric(args[4])
peakPick.minsd = as.numeric(args[5])
out.dir = args[6]

cat('Reading sliding window sample count...\n')
x0 = readRDS(in.count.file)
...
for (chr in chrs){
    ...
    x$peak[x$pct.samples < 5] = F
    print(table(x$peak))
    z = groupPeaks(x, d=100000)
